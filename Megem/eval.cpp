...
VALUE Console_print(ARRAY *arguments) {
	if (arguments->length > 0) PrintValue((VALUE *)Array_Get(arguments, 0), 0);
	return Value_Number(0);
}
...
VALUE Date_now(ARRAY *arguments) {
	return Value_Number(timeGetTime());
}

VALUE Date_getMilliseconds(ARRAY *arguments) {
	SYSTEMTIME time;
	GetSystemTime(&time);
	return Value_Number(time.wMilliseconds);
}

VALUE Date_getSeconds(ARRAY *arguments) {
	SYSTEMTIME time;
	GetSystemTime(&time);
	return Value_Number(time.wSecond);
}

VALUE Date_getMinutes(ARRAY *arguments) {
	SYSTEMTIME time;
	GetSystemTime(&time);
	return Value_Number(time.wMinute);
}

VALUE Date_getHours(ARRAY *arguments) {
	SYSTEMTIME time;
	GetSystemTime(&time);
	return Value_Number(time.wHour);
}

VALUE Date_getDate(ARRAY *arguments) {
	SYSTEMTIME time;
	GetSystemTime(&time);
	return Value_Number(time.wDay);
}

VALUE Date_getDay(ARRAY *arguments) {
	SYSTEMTIME time;
	GetSystemTime(&time);
	return Value_Number(time.wDayOfWeek);
}

VALUE Date_getMonth(ARRAY *arguments) {
	SYSTEMTIME time;
	GetSystemTime(&time);
	return Value_Number(time.wMonth);
}

VALUE Date_getYear(ARRAY *arguments) {
	SYSTEMTIME time;
	GetSystemTime(&time);
	return Value_Number(time.wYear);
}

void ResetStack() {
	if (stack) Array_Free(stack);
	stack = Array_New(sizeof(VALUE));
}
...
void SetupLibraries() {
	...
	Value_SetProperty(Console, "log", &Value_Compiled_Function(&Console_println));
	...
	VALUE *Date = Stack_Push("Date", &Value_Number(0), 0);
	Value_SetProperty(Date, "now", &Value_Compiled_Function(&Date_now));
	Value_SetProperty(Date, "getMilliseconds", &Value_Compiled_Function(&Date_getMilliseconds));
	Value_SetProperty(Date, "getSeconds", &Value_Compiled_Function(&Date_getSeconds));
	Value_SetProperty(Date, "getMinutes", &Value_Compiled_Function(&Date_getMinutes));
	Value_SetProperty(Date, "getHours", &Value_Compiled_Function(&Date_getHours));
	Value_SetProperty(Date, "getDate", &Value_Compiled_Function(&Date_getDate));
	Value_SetProperty(Date, "getDay", &Value_Compiled_Function(&Date_getDay));
	Value_SetProperty(Date, "getMonth", &Value_Compiled_Function(&Date_getMonth));
	Value_SetProperty(Date, "getYear", &Value_Compiled_Function(&Date_getYear));
...
VALUE Value_RawString(char *string_) {
	VALUE value = { 0 };
...
void PrintValue(VALUE *value, char quotes) {
	if (!value) {
		return;
	}
	...
	switch (value->type) {
		...
		case VALUE_ARRAY: {
			printf("[");
			for (unsigned int i = 0; i < value->array->length; ++i) {
				PrintValue((VALUE *)Array_Get(value->array, i), 1);
